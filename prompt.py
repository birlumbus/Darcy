...
def run_tests_for_model(category, model_id, output_text, base_outputs, dialogue_references):
    """
    Computes BLEU, ROUGE, and METEOR scores.
    
    For a base model (model_id "0"), it runs tests against dialogue_references.
    For a non-base model, it uses the base model's output (if available) along with dialogue_references.
    ...
    Returns:
        str: A formatted string containing the test metric results.
    """
    if model_id == "0":
        ...
        if category in base_outputs:
            ...
            return "\nBase model output not available for BLEU/ROUGE/METEOR comparison.\n"
...
def prompt_single_model(
    ...
    if dialogue_references:
        model_output_info += run_tests_for_model(category, model_id, output_text, base_outputs, dialogue_references)
